...
*Protected areas were downloaded from the [World Database on Protected Areas (WDPA)](https://www.protectedplanet.net/en/thematic-areas/wdpa?tab=WDPA). You can click on a protected area on the map to view its metadata*
```{r out.width = "80%", echo = FALSE, warning = FALSE, message=FALSE}
currSuit <- raster('data/analyses/current900.tif')

pa <- readRDS("data/analyses/protectedAreas.RDS")
pa$AREA_KM <- round(pa$AREA_KM, digits = 2)
...
leaflet() %>% addTiles() %>%
  addRasterImage(currProj, colors = pal, opacity = 0.85, maxBytes = Inf, project = FALSE, group = "Habitat Suitability") %>%
  addPolygons(data = pa, color = "blue", group = "Protected Areas",
              popup = paste("Name:", pa$NAME, "<br>",
                            "Designation:", pa$DESIG, "<br>",
                            "Ownership Type:", pa$OWN_TYP, "<br>",
                            "Managment Authority:", pa$MANG_AU, "<br>",
                            "Area:", pa$AREA_KM, "km<sup>2</sup>")) %>% 
  addLegend(
  ...
  addLayersControl(overlayGroups = c("Habitat Suitability", "Protected Areas"),
                  options = layersControlOptions(collapsed = FALSE)) %>% 
  hideGroup("Protected Areas")

```
*Note: Click on a protected area to view its metadata*
...
leaflet() %>% addTiles() %>%
  addRasterImage(futProj, colors = pal, opacity = 0.85, maxBytes = Inf, project = FALSE, group = "Habitat Suitability") %>%
  addPolygons(data = pa, color = "blue", group = "Protected Areas",
              popup = paste("Name:", pa$NAME, "<br>",
                            "Designation:", pa$DESIG, "<br>",
                            "Ownership Type:", pa$OWN_TYP, "<br>",
...
leaflet() %>% addTiles() %>% 
  addRasterImage(changeProj, colors = pal2, opacity = 0.85, maxBytes = Inf, group = "Change in Suitability") %>%
  addPolygons(data = pa, color = "blue", group = "Protected Areas",
              popup = paste("Name:", pa$NAME, "<br>",
                            "Designation:", pa$DESIG, "<br>",
                            "Ownership Type:", pa$OWN_TYP, "<br>",
